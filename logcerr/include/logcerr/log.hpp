...
#ifndef LOGCERR_LOG_HPP_INCLUDED
#define LOGCERR_LOG_HPP_INCLUDED
...
///
/// Define NDEBUG or LOGCERR_DISABLE_DEBUGGING to disable debug entries,
/// LOGCERR_ENABLE_DEBUGGING to enable debug entries.
/// Precedence order:
/// NDEBUG < LOGCERR_ENABLE_DEBUGGING < LOGCERR_DISABLE_DEBUGGING
[[nodiscard]] constexpr bool debugging_enabled() noexcept {
#if !defined(LOGCERR_DISABLE_DEBUGGING) && \
    (!defined(NDEBUG) || defined(LOGCERR_ENABLE_DEBUGGING))
...
/// Print a message to std::cerr visible and formatted according to its severity level.
template<typename... Args>
void print(severity level, format_string<Args...> fmt, Args&&... args) {
  if (is_outputted(level)) {
    impl::print(level,
                logcerr::format(std::move(fmt), std::forward<Args>(args)...));
  }
}
...
#endif // LOGCERR_LOG_HPP_INCLUDED
